...
| 🧾 **发票结算** | ✅ 生产就绪 | 100% | 专业发票生成，支持多币种、自定义字段、历史管理 |
...
### V1.3.4 - 单据管理中心小屏适配优化 (2025-01-07)
- **📱 Tab标签小屏适配**: 全面优化单据管理中心的tab标签在小屏设备上的显示
  - 响应式间距设计：小屏`space-x-1`，中屏`space-x-4`，大屏`space-x-8`
  - 智能文字显示：极小屏仅图标，小屏显示短名称，大屏显示完整名称
  - 图标和徽章缩放：小屏使用更小的图标(`h-3 w-3`)和徽章尺寸
  - 横向滚动支持：添加`overflow-x-auto`和`scrollbar-hide`确保所有tab可访问
- **🎨 UI细节优化**: 提升小屏设备的用户体验
  - 字体大小响应式：小屏`text-xs`，大屏`text-sm`
  - 内边距自适应：小屏`px-1`，大屏`px-2`
  - 徽章位置调整：确保在不同尺寸下的视觉平衡
  - 防止换行：添加`whitespace-nowrap flex-shrink-0`保持布局稳定
- **🔧 技术改进**: 
  - 新增`xs`断点配置（475px）提供更精细的响应式控制
  - 添加`scrollbar-hide`CSS类隐藏滚动条，提供更清洁的视觉效果
  - 优化容器边距：小屏使用`px-2`减少空间浪费
...
LC APP是一个专业的商业文档管理系统，专门用于处理日常业务中的各类商业文档，包括报价单、销售确认单、发票、采购单和装箱单等。系统支持多用户协作、权限管理，并提供AI辅助功能，大大提升工作效率。

### 主要特点
- 🚀 一站式文档管理：支持多种商业文档的生成和管理
- 📱 全设备支持：完美适配电脑、平板和手机
- 🌙 深色模式：支持明暗主题自动切换
- 🔒 安全可靠：本地数据存储，保护商业信息安全
- 🤖 AI助手：集成智能邮件助手，提供智能写作建议
...
### 2. 基本操作流程
- **创建文档**：点击顶部导航栏选择要创建的文档类型
- **填写信息**：按提示填写客户信息和商品明细
- **预览导出**：点击预览按钮查看效果，确认无误后导出PDF
- **查看历史**：在"历史记录"中管理所有已创建的文档
...
- **位置**：顶部导航栏 "装箱单"
- **特色功能**：
  - 三合一文档生成（装箱单/形式发票/合并文档）
  - 智能唛头管理
  - 自动单位转换
  - 包装信息管理

**使用步骤**：
1. 选择文档类型（装箱单/形式发票/合并）
2. 填写收货人信息
3. 添加商品明细和包装信息
4. 设置唛头信息
5. 预览并导出
...
### 1. 数据安全问题
Q: 我的数据存储在哪里？  
A: 系统采用本地存储方式，所有业务数据都保存在您的浏览器中，不会上传到服务器。

### 2. 浏览器兼容性
Q: 支持哪些浏览器？  
A: 推荐使用Chrome、Firefox、Safari等现代浏览器，并保持最新版本。

### 3. 移动端使用
Q: 手机可以使用吗？  
A: 可以。系统完全支持移动端，并针对触摸操作做了优化。
...
如果您在使用过程中遇到问题或有建议反馈，请联系系统管理员或技术支持。
