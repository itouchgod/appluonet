    ...
    input.onchange = async (e) => {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          if (jsonData) {
            try {
              const success = importInvoiceHistory(jsonData);
              if (success) {
                // 重新加载历史记录
                const results = getInvoiceHistory();
                setHistory(results);
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-[#3A3A3C]">
                        ...
                        <th className="w-[45%] sm:w-1/3 px-2 sm:px-4 py-3 text-left text-xs sm:text-sm font-medium text-gray-500 dark:text-[#98989D]">发票号</th>
                    ...
                    <tbody>
                      {history.map((item) => (
                        <tr key={item.id} className="border-b border-gray-200 dark:border-[#3A3A3C] last:border-0 hover:bg-gray-50 dark:hover:bg-[#3A3A3C]">
                          ...
                          <td className="w-[45%] sm:w-1/3 px-2 sm:px-4 py-2.5 text-xs sm:text-sm text-gray-900 dark:text-[#F5F5F7] whitespace-nowrap overflow-hidden text-ellipsis">{item.invoiceNo}</td>
