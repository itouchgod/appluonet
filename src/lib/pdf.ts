...
const loadFonts = async (doc: jsPDF) => {
  // 加载中文字体
  const notoSansSCNormalResponse = await fetch('/fonts/NotoSansSC-Regular.ttf');
  const notoSansSCNormalFont = await notoSansSCNormalResponse.arrayBuffer();
  doc.addFileToVFS('NotoSansSC-Regular.ttf', notoSansSCNormalFont);
  doc.addFont('NotoSansSC-Regular.ttf', 'NotoSansSC', 'normal');

  // 加载中文粗体
  const notoSansSCBoldResponse = await fetch('/fonts/NotoSansSC-Bold.ttf');
  const notoSansSCBoldFont = await notoSansSCBoldResponse.arrayBuffer();
  doc.addFileToVFS('NotoSansSC-Bold.ttf', notoSansSCBoldFont);
...
export const generateInvoicePDF = async (data: QuotationData) => {
  ...
  let currentY = customerInfoStartY;
...
// 导出报价单生成函数
export const generateQuotationPDF = async (data: QuotationData, type: 'quotation' | 'confirmation') => {
  const doc = new jsPDF();
  ...
  doc.setFont('NotoSansSC', 'bold');
  doc.setFontSize(24);
  doc.text(type === 'quotation' ? 'QUOTATION' : 'ORDER CONFIRMATION', pageWidth / 2, 45, { align: 'center' });
  ...
  doc.text('Quotation No.', labelX, 55, { align: 'right' });
  doc.text(':', colonX, 55);
  ...
  if (type === 'confirmation') {
    doc.text('Contract No.', labelX, 65, { align: 'right' });
    doc.text(':', colonX, 65);
    doc.text(data.contractNo || '', valueX, 65);
  }
  ...
  autoTable(doc, {
    startY: tableStartY,
    head: [['No.', 'Part Name', 'Description', 'Q\'TY', 'Unit', 'Unit Price', 'Amount']],
    ...
    foot: [[
      ...
      { 
        content: data.items.reduce((sum, item) => sum + item.amount, 0).toFixed(2),
        styles: { 
          fontStyle: 'bold',
          halign: 'center',
          valign: 'middle'
  ...
  // 添加注意事项
  const notesStartY = finalY + 10 + (lines.length * 5) + 5;
  doc.setFont('NotoSansSC', 'normal');

  data.notes.forEach((note, index) => {
    doc.text(`${index + 1}. ${note}`, margin, notesStartY + (index * 5));
  });

  // 保存文件
  doc.save(`${type === 'quotation' ? 'Quotation' : 'Order'}_${data.quotationNo}_${data.date}.pdf`);
};

// 导出订单确认函数
export const generateOrderConfirmationPDF = async (data: QuotationData) => {
  return generateQuotationPDF(data, 'confirmation');
}; 